<!DOCTYPE html>
<html lang="id">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Panel Creator - Public/Private</title>
    <link rel="stylesheet" href="style.css">
    <style>
        /* --- Ban Popup/Modal CSS --- */
        .modal-overlay {
            position: fixed;
            top: 0;
            left: 0;
            width: 100%;
            height: 100%;
            background: rgba(0, 0, 0, 0.8);
            display: none;
            justify-content: center;
            align-items: center;
            z-index: 1000;
        }
        .modal-content {
            background: var(--container-bg);
            border: 2px solid var(--error-color);
            border-radius: 12px;
            padding: 30px;
            box-shadow: 0 0 20px rgba(248, 81, 73, 0.4);
            width: 90%;
            max-width: 450px;
            animation: modalFadeIn 0.3s ease-out forwards;
            position: relative;
        }
        .modal-content h3 {
            font-family: 'Orbitron', sans-serif;
            color: var(--error-color);
            text-align: center;
            margin-top: 0;
            text-shadow: 0 0 8px rgba(248, 81, 73, 0.3);
        }
        .modal-content p {
            margin: 10px 0;
            line-height: 1.5;
        }
        .modal-content p strong {
            color: var(--text-color);
        }
        .modal-content .close-button {
            background: none;
            border: none;
            color: var(--placeholder-color);
            font-size: 1.5em;
            position: absolute;
            top: 10px;
            right: 15px;
            cursor: pointer;
            transition: color 0.2s;
        }
        .modal-content .close-button:hover {
            color: var(--text-color);
        }

        @keyframes modalFadeIn {
            from { opacity: 0; transform: scale(0.9); }
            to { opacity: 1; transform: scale(1); }
        }
        /* --- End Ban Popup CSS --- */
    </style>
</head>
<body>
    <div class="background-particles">
        <div class="particle" style="width: 60px; height: 60px; left: 10%; top: 10%;"></div>
        <div class="particle" style="width: 40px; height: 40px; left: 80%; top: 30%;"></div>
        <div class="particle" style="width: 90px; height: 90px; left: 20%; top: 80%;"></div>
        <div class="particle" style="width: 70px; height: 70px; left: 60%; top: 5%;"></div>
        <div class="particle" style="width: 55px; height: 55px; left: 5%; top: 40%;"></div>
    </div>

    <div id="maintenance-view" style="display: none;">
        <i class="fas fa-tools maintenance-icon"></i>
        <h1>WEBSITE SEDANG DALAM PERBAIKAN</h1>
        <p>Mohon maaf atas ketidaknyamanannya. Kami sedang melakukan pembaruan dan perbaikan penting untuk meningkatkan layanan kami. Silakan coba kembali dalam beberapa saat.</p>
    </div>

    <div id="main-view" style="display: none;">
        <div class="container">
            <h1>CREATE PANEL</h1>
            <form id="createPanelForm">
                <div class="form-group">
                    <label for="accessKey">Access Key:</label>
                    <input type="text" id="accessKey" name="accessKey" placeholder="Masukkan Access Key Anda" class="input-glow-border" required>
                    <div id="keyStatusMessage" class="key-status-message"></div>
                </div>

                <div class="form-group">
                    <label for="panelType">Tipe Panel:</label>
                    <select id="panelType" name="panelType" class="input-glow-border" required>
                        <option value="" disabled selected>Pilih Tipe Panel</option>
                    </select>
                </div>

                <div class="form-group">
                    <label for="username">Username:</label>
                    <input type="text" id="username" name="username" placeholder="Contoh: user_keren2025" class="input-glow-border" required minlength="4" maxlength="16" pattern="^[a-zA-Z0-9_.-]+$" title="Hanya huruf, angka, underscore, titik, atau dash. Minimal 4, Maksimal 16 karakter.">
                </div>

                <div class="form-group">
                    <label for="hostingPackage">Paket Hosting:</label>
                    <select id="hostingPackage" name="hostingPackage" class="input-glow-border" required>
                        <option value="" disabled selected>Pilih Paket</option>
                        <option value="unlimited">Unlimited</option>
                        <option value="10gb">10 GB</option>
                        <option value="9gb">9 GB</option>
                        <option value="8gb">8 GB</option>
                        <option value="7gb">7 GB</option>
                        <option value="6gb">6 GB</option>
                        <option value="5gb">5 GB</option>
                        <option value="4gb">4 GB</option>
                        <option value="3gb">3 GB</option>
                        <option value="2gb">2 GB</option>
                        <option value="1gb">1 GB</option>
                    </select>
                </div>

                <button type="submit" id="createButton" class="cyber-button">
                    <span>Create Panel</span> <span class="loader-dots" id="loadingSpinner">
                        <span class="dot"></span>
                        <span class="dot"></span>
                        <span class="dot"></span>
                    </span>
                    <div class="loading-progress-bar"></div> </button>
            </form>

            <div id="responseMessage" class=""></div>

        </div>

        <div id="toast-notification-container"></div>
        
        <div id="banModal" class="modal-overlay">
            <div class="modal-content">
                <button class="close-button">&times;</button>
                <h3>ACCOUNT BANNED</h3>
                <p id="banReason"></p>
                <p id="banTime"></p>
                <p id="contactAdmin"></p>
            </div>
        </div>
    </div> <script src="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/5.15.4/js/all.min.js"></script>
    <script src="main.js" type="module"></script>
</body>
</html>
